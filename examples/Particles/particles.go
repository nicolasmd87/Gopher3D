...
func (pb *ParticleBehaviour) Start() {
	...
	for i := 0; i < numParticles; i++ {
		position := mgl.Vec3{
			rand.Float32()*650 - 10, // X: random between -10 and 10
			rand.Float32()*650 - 10, // Y: random between -10 and 10
			rand.Float32()*650 - 10, // Z: random between -10 and 10
		}
		color := randomColor()
...
func (pb *ParticleBehaviour) createParticle(position mgl.Vec3, color string) {
	...
	case "red":
		...
		m.SetSpecularColor(255.0, 0.0, 0.0)
	case "yellow":
		m.SetDiffuseColor(255.0, 255.0, 0.0)
		m.SetSpecularColor(255.0, 255.0, 0.0)
	case "green":
		m.SetDiffuseColor(0.0, 255.0, 0.0)
		m.SetSpecularColor(0.0, 255.0, 0.0)
	case "blue":
		m.SetDiffuseColor(0.0, 0.0, 255.0)
		m.SetSpecularColor(0.0, 0.0, 255.0)
	}
...
func (pb *ParticleBehaviour) calculateForce(p1, p2 *Particle) mgl.Vec3 {
	...
	case "red":
		if p2.color == "yellow" {
			magnitude = 50.0 // Attraction force
		} else if p2.color == "green" {
			magnitude = -50.0 // Repulsion force
